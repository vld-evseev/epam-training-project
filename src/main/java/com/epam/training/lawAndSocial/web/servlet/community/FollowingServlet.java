...
@Singleton
public class FollowingServlet extends AbstractCommunityServlet {

    private final static Logger LOGGER = LoggerFactory.getLogger(FollowingServlet.class);
    private final FollowService followService;

    @Inject
    public FollowingServlet(FollowService followService) {
        super(followService);
    ...
    protected List<User> getRequestedUsers(long userId, int currentPageNumber) {
        ...
        return followService.getFollowing(userId, currentOffset);
    }

    @Override
    protected void setPathAttribute(HttpServletRequest req) {
        req.setAttribute("path", "/user/following");
    }
}
